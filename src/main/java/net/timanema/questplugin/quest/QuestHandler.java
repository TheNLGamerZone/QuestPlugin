...
public class QuestHandler
{
    ...
    public void initQuests(Set<UUID> questUUIDs)
    {
        QuestPlugin.getLog().info("Loading quests");

        // Loop over ids
        for (UUID uuid : questUUIDs)
        {
            Quest quest = this.questPlugin.getQuestImageBuilder().load(uuid);

            // Check if quest failed to load
            if (quest == null || quest.checkBroken())
            {
                quest = new Quest(uuid, null, null, null, null,
                        null, false, false, true, false, false, true);

                QuestPlugin.getLog().warning("Quest '" + uuid + "' is broken/missing dependencies for now, " +
                        "will wait for possible dependencies before trying again!");
            }
    ...
    public void initFloatingStages(Set<UUID> stageUUIDs)
    {
        ...
        for (UUID uuid : this.questPlugin.getStorage().searchForObject(Storage.DataType.STAGE, stageUUIDs,
                new Storage.DataPair<>("quest", "null")))
        {
            Stage stage = this.questPlugin.getStageImageBuilder().load(uuid);

            // Check if stage failed to load
            if (stage == null || stage.checkBroken())
            {
                stage = new Stage("broken_stage_id_unknown", null, uuid, null, true, false, false);

                QuestPlugin.getLog().warning("Stage '" + uuid + "' is broken/missing dependencies for now, " +
                        "will wait for possible dependencies before trying again!");
            }
